...
🛑 警告：本项目遵循 MIT 协议。任何对项目的修改和发布必须保留原作者署名。

本项目禁止传播，有缘人自会看到。给个星标再 fork 吧，求求了

本仓库严厉谴责 2pacJay/Quark_Auot_Check_In 仓库 抹除原作者署名权的行为，该行为严重违反 MIT 协议，侵害开源精神！
...
感谢 Spectrollay 对工作流的优化
感谢 haozihong 对工作流的优化
📋 使用指南
1️⃣ Fork 项目
...
🛠️ 获取 COOKIE_QUARK
使用手机抓包工具（小白推荐 proxypin）获取 Cookie 信息：

打开手机抓包工具，访问夸克网盘签到页。

找到接口 https://drive-m.quark.cn/1/clouddrive/capacity/growth/info 的请求信息。

复制请求中的参数：kps、sign 和 vcode。【初步测试发现这个 Key 的值有效期在两个月左右】
...
将整理好的 Cookie 信息粘贴到 "Secret" 输入框中并保存。
...
启用后，你会看到名为 Quark签到 (或 Quark Sign-in) 的工作流已配置完成。
脚本将按预设时间（北京时间每日约 9:00 和 13:00）自动运行。
...
4️⃣ 手动测试运行
进入 Actions 选项卡，点击左侧的 Quark签到 (或 Quark Sign-in) 工作流。
点击右侧的 Run workflow 按钮，然后再次点击绿色 Run workflow 按钮，手动触发任务以验证配置是否成功。
你可以点击运行中的 workflow 查看其执行日志和状态。
❓ 常见问题
签到失败：
...
GitHub Actions 未生效或报错 Permission denied / GH006：
